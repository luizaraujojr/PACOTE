	...
	#Diretório da base de dados observados a ser lida
		dataDir <- 'D:\\Backup\\eclipse-workspace\\projetotese\\results\\graph\\jhotdraw'
	...
	for (i in 1:length(larquivos)){
		...
		text(0,-1.4,  paste(month(date_version), year(date_version),sep="/"), col="black", cex=2)
	 	...
	 	library(igraph)
	library("lubridate")
	par(mfrow = c(7,6), cex=0.2)
	
	#Diretório da base de dados observados a ser lida
		dataDir <- 'D:\\Backup\\eclipse-workspace\\projetotese\\results\\graph\\jedit'
