...
//	private boolean[] metricasUtilizadas;
	 ...
	 */
	public ClusterMetrics3(ModuleDependencyGraph mdg, int[] solution, int[] equationParams, Project project, boolean[] usedMetrics)
	{
		this.mdg = mdg;
...
//			int joinClusterExternalDependency,	
	...
	public double calculateClusterFitness(int _numberInternalDependencies, int _numberInternalClassWithDepOutCluster, int _numberExternalClassWithDepCluster)
	{
		...
		double ra5 = c[2] * _numberExternalClassWithDepCluster;
		
		
		double fitness = ra1 + ra4 + ra5;	
		if (fitness != 0.0) 
		{
			double rb1 = c[3] * _numberInternalDependencies;
			double rb4 = c[4] * _numberInternalClassWithDepOutCluster;
			double rb5 = c[5] * _numberExternalClassWithDepCluster;
			double rb = rb1 + rb4 + rb5;			
			fitness /= rb;
		}
		return fitness;
	}
