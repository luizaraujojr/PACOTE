...
//	private boolean[] metricasUtilizadas;
	 ...
	 */
	public ClusterMetrics4(ModuleDependencyGraph mdg, int[] solution, int[] equationParams, Project project, boolean[] usedMetrics)
	{
		this.mdg = mdg;
...
//			this.internalClassesInternalDependencies[cluster] = countICID;
	...
	{		
		...
		double fitness = calculateClusterFitness(joinClusterInternalDependency, joinClusterExternalDependency, joinClusterInternalClassesWithExternalDependency, joinClusterExternalClassesWithInternalDependency);
		fitness -= calculateClusterFitness(firstInternalDependencyWeightCluster, firstExternalDependencyWeightCluster, firstClusterInternalClassesWithExternalDependency, firstClusterExternalClassesWithInternalDependency);
		fitness -= calculateClusterFitness(secondInternalDependencyWeightCluster, secondExternalDependencyWeightCluster, secondClusterInternalClassesWithExternalDependency, secondClusterExternalClassesWithInternalDependency);
		return fitness;
	}

		
	public double calculateClusterFitness(int _numberInternalDependencies, int _numberExternalDependencies, int _numberInternalClassWithDepOutCluster, int _numberExternalClassWithDepCluster)
	{
		double ra1 = c[0] * _numberInternalDependencies;
		double ra2 = c[1] * _numberExternalDependencies;
		double ra4 = c[2] * _numberInternalClassWithDepOutCluster;
		double ra5 = c[3] * _numberExternalClassWithDepCluster;
		
		
		double fitness = ra1 + ra2 + ra4 + ra5;	
		if (fitness != 0.0) 
		{
			double rb1 = c[4] * _numberInternalDependencies;
			double rb2 = c[5] * _numberExternalDependencies;
			double rb4 = c[6] * _numberInternalClassWithDepOutCluster;
			double rb5 = c[7] * _numberExternalClassWithDepCluster;
			double rb = rb1 + rb2 + rb4 + rb5;			
			fitness /= rb;
		}
		return fitness;
	}
...
//	 */
	public double calculateFitness()
	{
		...
		for (int clusterNumber = 0; clusterNumber < classCount; clusterNumber++) 
		{
			...
			fitness += calculateClusterFitness(internalDependencyWeight[_clusterNumber], externalDependencyWeight[_clusterNumber], internalClassesExternalDependencies [_clusterNumber], externalClassesInternalDependencies [_clusterNumber]); 
		}
