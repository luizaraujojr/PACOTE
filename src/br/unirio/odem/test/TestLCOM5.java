...
public class TestLCOM5
{
	private static void testaPrimeiroProjeto() throws Exception
	{
		...
		if (Math.abs(cc.calculateLCOM5() - 0.8333) > 0.0001) throw new Exception("Erro no c�lculo do LCOM5");
	}
	...
	private static void testaSegundoProjeto() throws Exception
	{
		...
		ClusteringCalculator cc = new ClusteringCalculator(project, project.getPackageCount());
		if (Math.abs(cc.calculateLCOM5() - 0.75) > 0.0001) throw new Exception("Erro no c�lculo do LCOM5");
	}
	...
	private static void testaTerceiroProjeto() throws Exception
	{
		...
		ClusteringCalculator cc = new ClusteringCalculator(project, project.getPackageCount());
		if (Math.abs(cc.calculateLCOM5() - 0.6666) > 0.0001) throw new Exception("Erro no c�lculo do LCOM5");
	}
	...
	private static void testaQuartoProjeto() throws Exception
	{
		...
		ClusteringCalculator cc = new ClusteringCalculator(project, project.getPackageCount());
		if (Math.abs(cc.calculateLCOM5() - 0.0) > 0.0001) throw new Exception("Erro no c�lculo do LCOM5");
	}
